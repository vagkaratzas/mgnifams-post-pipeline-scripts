...
def check_versions_file(subfolder_path):
    ...
    if os.path.isfile(versions_file_path):
        try:
            if "FOLDSEEK_EASYSEARCH" in open(versions_file_path).read():
                return True
        except Exception as e:
            print(f"An error occurred while reading {versions_file_path}: {e}")
    return False

def concat_m8(second_level_subfolder, output_file):
    # Find all files matching the pattern *_scores.txt in the second_level_subfolder
    m8_files = glob.glob(os.path.join(second_level_subfolder, '*.m8'))

    # Open the output_file in append mode
    with open(output_file, 'a') as out_f:
        # Iterate through each m8 file
        for m8_file in m8_files:
            # Open each m8 file and append its content to the output_file
            with open(m8_file, 'r') as in_f:
        ...
        for subfolder in first_level_subfolders:
            ...
            for second_level_subfolder in second_level_subfolders:
                if check_versions_file(second_level_subfolder):
                    ...
                    concat_m8(second_level_subfolder, output_file)
    except Exception as e:
...
def main():
    ...
    # Example call local: ./bin/extract_foldseek_m8s.py /home/vangelis/Desktop/Projects/mgnifams/work output/foldseek_hits.tsv
